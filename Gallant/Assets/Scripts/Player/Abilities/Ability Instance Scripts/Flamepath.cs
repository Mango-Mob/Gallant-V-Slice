...
public class Flamepath : MonoBehaviour
{
    ...
    public List<GameObject> flameParticles { get; private set; } = new List<GameObject>();
    ...
    void FixedUpdate()
    {
        ...
        if (m_beganLife)
        {
            ...
            {
                foreach (var vfx in flameParticles)
                {
    ...
    private void OnTriggerStay(Collider other)
    {
        if (other.gameObject.layer == LayerMask.NameToLayer("Attackable"))
        {
            ...
            if (status != null)
            {
                status.AddStatusEffect(new BurnStatus(m_data.damage, m_data.duration));
            }
        }
    }
